...
class CubicSpline:
    ...
    def is_valid(self) -> bool:
        ...
        return all(jnp.isfinite(coeff) for coeff in [self.x0, self.a, self.b, self.c, self.d])
    ...
    def evaluate(self, x: Float) -> Float:
        ...
        delta = x - self.x0
        return self.a + self.b * delta + self.c * delta**2 + self.d * delta**3

    def is_quadratic(self) -> bool:
        """Check if spline is effectively quadratic matching C++ CubicSpline_IsQuadratic."""
        return jnp.abs(self.d) < LINESEARCH_TOL
...
def _quadratic_formula(a: Float, b: Float, c: Float) -> tuple[Float, Float, CubicSplineReturnCode]:
    ...
    if jnp.abs(a) < LINESEARCH_TOL:
        return jnp.nan, jnp.nan, CubicSplineReturnCode.IS_LINEAR
    ...
    if jnp.abs(discriminant) < LINESEARCH_TOL:
        ...
        return jnp.nan, jnp.nan, CubicSplineReturnCode.NO_MINIMUM

    sqrt_discriminant = jnp.sqrt(discriminant)
    x1 = (-b + sqrt_discriminant) / (2 * a)
    x2 = (-b - sqrt_discriminant) / (2 * a)

    return x1, x2, CubicSplineReturnCode.NO_ERROR
...
def cubic_spline_from_2_points(
    ...
    return CubicSpline(x1, a, b, c, d), CubicSplineReturnCode.NO_ERROR
...
def cubic_spline_from_3_points(
    ...
    return CubicSpline(x0, a, b, c, d), CubicSplineReturnCode.NO_ERROR
...
def quadratic_spline_from_2_points(
    ...
    return CubicSpline(x0, y0, d0, dy, 0.0), CubicSplineReturnCode.NO_ERROR
...
def cubic_spline_argmin(spline: CubicSpline) -> tuple[Float, CubicSplineReturnCode]:
    ...
    if not spline.is_valid():
        return jnp.nan, CubicSplineReturnCode.INVALID_POINTER
    ...
    if is_constant:
        return jnp.nan, CubicSplineReturnCode.IS_CONSTANT
    elif is_linear:
        return jnp.nan, CubicSplineReturnCode.IS_LINEAR
    elif is_quadratic:
        if c <= 0:
            return jnp.nan, CubicSplineReturnCode.IS_POSITIVE_QUADRATIC
        else:
            return -b / (2 * c) + spline.x0, CubicSplineReturnCode.FOUND_MINIMUM

    # Find stationary points for cubic
    d1, d2, err_code = _quadratic_formula(3 * d, 2 * c, b)

    if err_code != CubicSplineReturnCode.NO_ERROR:
        return jnp.nan, err_code
    ...
    if jnp.abs(curv1) < LINESEARCH_TOL and jnp.abs(curv2) < LINESEARCH_TOL:
        return jnp.nan, CubicSplineReturnCode.SADDLE_POINT
    elif curv1 > 0 and curv2 < 0:
        return x1, CubicSplineReturnCode.FOUND_MINIMUM
    elif curv1 < 0 and curv2 > 0:
        return x2, CubicSplineReturnCode.FOUND_MINIMUM
    else:
        return jnp.nan, CubicSplineReturnCode.UNEXPECTED_ERROR
